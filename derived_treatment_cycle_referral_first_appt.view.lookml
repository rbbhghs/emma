- view: derived_treatment_cycle_referral_first_appt
  derived_table:
    sql: |
      ...
      FROM
      (select patient_id,treatment_cycle_referral_id, min(start) first_appt, from appointment where ifnull(dna,0) = 0 and status = 'A' group by patient_id,treatment_cycle_referral_id)x 
      JOIN appointment a ON (x.patient_id = a.patient_id and x.first_appt = a.start and x.treatment_cycle_referral_id = a.treatment_cycle_referral_id) 
      JOIN treatment_cycle_referral tcr ON (tcr.treatment_cycle_referral_id = a.treatment_cycle_referral_id)
      GROUP BY a.patient_id,a.appointment_id,first_appt,a.treatment_cycle_referral_id,tcr.created_date,tcr.referred 
      ;
