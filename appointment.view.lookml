...
#    sql: CASE WHEN ${TABLE}.arrive is null then 1 else 0 End
  ...
  - dimension: appointment_status
    sql: case when ${TABLE}.status = 'A' then 'Active' when ${TABLE}.status in ('C','N') then 'Cancelled' when ${TABLE}.status = 'R' then 'Rescheduled' else 'Unknown' End

  - dimension: appointment_status_description
    sql: case when ${TABLE}.status = 'A' then 'Active' when ${TABLE}.status = 'C' then 'Patient Cancelled' when ${TABLE}.status = 'N' then 'User Cancelled' when ${TABLE}.status = 'R' then 'Rescheduled' else 'Unknown' End
...
#    drill_fields: [appointment_id, individual.forename, start_time, end_time]
  ...
  - measure: appt_duration_mins
    label: 'Appointment Duration in Minutes'
    type: sum
    sql: TIMESTAMPDIFF(MINUTE,${TABLE}.start,${TABLE}.end)
    value_format: '#,##0'     
  ...
  - measure: actual_appt_duration_mins
    label: 'Actual Appointment Duration in Minutes'
    type: sum
    #sql: TIMESTAMPDIFF(SECOND,ifnull((${TABLE}.view),(${TABLE}.start)),ifnull((${TABLE}.leave),(${TABLE}.end)))/60.00
    sql: TIMESTAMPDIFF(MINUTE,${TABLE}.view,${TABLE}.leave)    
    value_format: '#,##0'   
  ...
  - measure: time_to_view_mins
    label: 'Time to be Seen in Minutes'
    type: sum
    sql: TIMESTAMPDIFF(MINUTE,${TABLE}.start,${TABLE}.view)    
    value_format: '#,##0'
